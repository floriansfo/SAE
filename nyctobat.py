...
class Nyctobat(Monstre):
    ...
    def affichage(self, ecran, camx, camy):
        if self.direction== Direction.face:
            frames = assets.ASSETS['nyctobat_face']
        elif self.direction==Direction.droite:
            frames= assets.ASSETS['nyctobat_droite']
        elif self.direction ==Direction.gauche:
            frames = assets.ASSETS['nyctobat_gauche']
        else:
            frames= assets.ASSETS['nyctobat_back']
        self.time +=1
        if(self.time> self.vitesseanim):
            self.time=0
            self.animation=(self.animation+1)% len(frames)
        nyctojpeg= frames[self.animation]
        rect= nyctojpeg.get_rect(center = (self.rect.centerx + camx, self.rect.centery + camy))
        ecran.blit(nyctojpeg, rect)
